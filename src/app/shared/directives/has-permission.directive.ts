...
export class HasPermissionDirective implements OnInit, OnDestroy {
  ...
  private permissionsToCheck: (Permission | string)[] = [];
  private checkMode: 'any' | 'all' = 'all';
  
  @Input() set hasPermission(permission: Permission | string | (Permission | string)[]) {
  ...
  private updateView(): void {
    ...
    // 根據模式選擇檢查方法
    const checkObservable = this.checkMode === 'any'
...
export class HasRoleDirective implements OnInit, OnDestroy {
  ...
  private updateView(): void {
    ...
    const checkObservable = this.checkMode === 'any'
      ...
      : this.rbacService.hasRole(this.rolesToCheck[0]);  // 單一角色
